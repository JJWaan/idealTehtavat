...
// get kysymys by id (works, id validation is under construction)
...
router.post('/', async (request, response) => {
    ...
    const values = [teksti];
    try {
        const sqlCommand = "INSERT INTO kysymys (kysymys_nimi) VALUES ($1)";
...
// update a single kysymys_kuvaus
router.put('/:id', async (request, response) => {
    ...
    try {
        ...
        const sqlCommand = "UPDATE kysymys SET kysymys_kuvaus=($1) WHERE kysymys_id=($2)";
        await pool.postgrePool().query(sqlCommand, values);
